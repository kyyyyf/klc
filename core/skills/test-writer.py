...
The skill:
- Reads .klc/index/test-framework.json (produced by the test agent).
- Reads .klc/index/modules.json for module path resolution.
...
def resolve_modules(names: list[str]) -> list[dict]:
    mods_doc = load_json(klc_index_dir() / "modules.json")
...
def main() -> int:
    ...
    fw_index = klc_index_dir() / "test-framework.json"
    if fw_index.exists():
