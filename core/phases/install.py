...
def _shim_source_cmd(fw: Path, project: Path) -> str:
    """Windows cmd.exe shim. Uses backslashes and %*.

    Pinning PROJECT_ROOT via `set` scopes it to the invoked python,
    not the parent shell — the caller's env is not polluted.
    """
    fw_win = str(fw).replace("/", "\\")
    pr_win = str(project).replace("/", "\\")
    return (
        "@echo off\r\n"
