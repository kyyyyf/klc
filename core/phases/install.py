...
def run(argv: list[str]) -> int:
    ...
    # --- shim --------------------------------------------------------------
    shim_path = bin_dir / "klc"
    shim_body = _shim_source(fw, project)
    if shim_path.exists() and not args.force:
        sys.stderr.write(
            f"klc install: {shim_path} already exists (use --force to "
            "overwrite)\n"
        )
    else:
    ...
    print(f"  klc shim:   {shim_path.relative_to(project)}")
    print(f"  profile:    {args.profile}")
    print(f"  mcp.json:   {'copied' if mcp_src.exists() else '(profile has no mcp.json)'}")
    print()
    print("Next steps (run from the project root):")
    print(f"  cd {project}")
    print( "  .klc/bin/klc doctor          # verify the install")
    print( "  ./klc-fw/scripts/install-deps.sh  # or wherever the klc repo lives:")
    print(f"  {fw}/scripts/install-deps.sh")
    print( "  .klc/bin/klc init            # bootstrap indices + per-module CLAUDE.md")
    return 0


# ---- helpers --------------------------------------------------------------

def _shim_source(fw: Path, project: Path) -> str:
    return (
        "#!/usr/bin/env bash\n"
        "# klc shim — generated by `klc install`.\n"
        "# Forwards every call to the klc repo at:\n"
        f"#   {fw}\n"
        "# with PROJECT_ROOT pinned to this project.\n"
        "#\n"
