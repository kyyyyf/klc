...
LOG_FILE="$KLC_DIR/logs/install-deps.log"
mkdir -p "$KLC_DIR/logs"
...
if have ast-grep || have sg; then
  ...
  rule_dirs=$(python3 "$FRAMEWORK_ROOT/core/skills/profile-resolve.py" --field rules)
  for rd in $rule_dirs; do
    for rf in "$FRAMEWORK_ROOT/$rd"/*.yaml; do
      [ -f "$rf" ] || continue
