...
# an LLM, the script writes a job card to .klc/reports/pending-<TS>/ that
# lists the prompt file + context bundle, and then waits for partial reports
# to appear in .klc/reports/partials-<TS>/. This keeps the script usable both
# from Claude Code (which fulfils the job cards) and from CI (where an
# operator or a wrapper is expected to execute the prompts).
#
...
CURRENT_PROFILE="$(python3 "$FRAMEWORK_ROOT/core/skills/profile-resolve.py" --field name 2>/dev/null || echo unknown)"
...
MODS_JSON="$KLC_DIR/index/modules.json"
AFFECTED_MODS="$PENDING_DIR/affected-modules.txt"
: > "$AFFECTED_MODS"
if [ -f "$MODS_JSON" ]; then
  python3 "$FRAMEWORK_ROOT/core/skills/diff-modules.py" "$DIFF_FILE" --modules "$MODS_JSON" \
...
REVIEWERS_JSON="$(python3 "$FRAMEWORK_ROOT/core/skills/profile-resolve.py" --field reviewers)"
...
for r in "${REVIEWERS[@]}"; do
  ...
  if [ -n "$filter" ]; then
    ...
    python3 "$FRAMEWORK_ROOT/core/skills/filter-diff.py" \
...
PY
    ...
    reviewer_mods=$(python3 "$FRAMEWORK_ROOT/core/skills/diff-modules.py" \
  ...
  # Allowlist lives in per-project knowledge; fall back to the framework
  # template on first run, before init.sh seeds the project copy.
  allowlist="$KLC_DIR/knowledge/reviewer-allowlist.yml"
  [ -f "$allowlist" ] || allowlist="$FRAMEWORK_ROOT/config/reviewer-allowlist.seed.yml"
  cat > "$card" <<EOF
