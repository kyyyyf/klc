...
def build_symbols_by_module(inventory: dict, modules: list[dict]) -> dict[str, list[dict]]:
    """Walk inventory.symbols once; assign each item to the module whose
    path is its longest matching prefix. O(symbols * log modules) instead
    of module_public_api's O(modules * symbols) when called per module.
    Modules are sorted by descending path length so the first prefix
    match is also the longest."""
...
def main() -> int:
    ...
    inventory = load_json(framework_root() / "index" / "inventory.json")
    modules_doc = load_json(framework_root() / "index" / "modules.json")
    modules = modules_doc.get("modules", [])

    _ = resolve_modules(requested, modules)  # validates names

    # Build the module → items index once. Dynamic-depth loops reuse it.
    by_module = build_symbols_by_module(inventory, modules)
