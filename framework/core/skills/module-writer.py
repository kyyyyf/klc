...
Usage:
    module-writer.py --root [--out CLAUDE.md]
        Render the root CLAUDE.md from framework/index/{inventory,modules}.json
...
def framework_root() -> Path:
    # The skill lives at framework/core/skills/; framework root is three levels up.
...
def render_root(out_path: Path | None) -> Path:
    inv = load_json(framework_root() / "index" / "inventory.json")
    mods = load_json(framework_root() / "index" / "modules.json")
    env = jinja_env()
...
def main() -> int:
    ...
    try:
        if args.root:
            ...
            (framework_root() / "index" / "modules.json").write_text(
...
def _write_inventory_hash() -> None:
    hash_path = framework_root() / "index" / "inventory-hash.json"
