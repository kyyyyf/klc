...
Reads `framework/index/structural.json` for `source_roots` and scans the
...
def load_structural() -> dict:
    p = framework_root() / "index" / "structural.json"
    if not p.exists():
...
def main() -> int:
    ...
    structural = json.loads((args.structural or (framework_root() / "index" / "structural.json")).read_text(encoding="utf-8"))
