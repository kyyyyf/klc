...
## Configuration
Read `framework/config/reviewers.yml` → `external_reviewer`:
...
external_reviewer:
  ...
  report_path:   framework/reports/external-review-{timestamp}.md
```
...
### 1. Build the prompt
Render `framework/core/templates/external-review-prompt.j2` with:
- `spec`               — from input
- `diff`               — from input
...
```json
{
  ...
  "path":     "framework/reports/external-review-2026-05-04-10-15.md"
}
```
