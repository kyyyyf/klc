...
and write `framework/index/modules.json`. Every later agent keys off this.
...
## Inputs
- `framework/index/inventory.json` (produced by the inventory agent).
...
- After step 6 emits `modules.json`, run
  `framework/core/skills/public-api-filter.py --cap 40`. The skill
  drops any forward declarations that slipped through, caps the list at
  40, and records `public_api_total` + `public_api_note` on truncated
  modules. This keeps `CLAUDE.md` files cheap to feed into reviewers.
...
- **A module has `symbol_count == 0` and shares its path with another
  ...
  `framework/core/skills/filter-build-overrides.py`. It moves such
...
### 6. Emit
Write `framework/index/modules.json`:
