...
- `claude_md_context` — root `CLAUDE.md` plus the `CLAUDE.md` of every
                        ...
                        via `framework/index/modules.json` — honour
...
### 1. Resolve inputs
- Load `framework/config/reviewers.yml`:
...
`framework/reports/<reviewer>-<timestamp>.partial.md`.
...
`framework/core/agents/external-review.md` with the same context.
Missing API key → log, skip, continue with internal-only verdict.

### 5. Aggregate
Render `framework/core/templates/review-report.md.j2` with:
...
Save to `framework/reports/review-<YYYY-MM-DD-HH-MM>.md`.
...
A reference runner ships as `framework/scripts/review-runner-claude.sh`
(Claude CLI). Usage:

```bash
RUN_LOCAL_SUBAGENTS=1 \
REVIEW_RUNNER="$PWD/framework/scripts/review-runner-claude.sh" \
./framework/scripts/review.sh --diff HEAD --spec framework/index/pending-feature.md
```
