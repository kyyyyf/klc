# Agent prompt — KLC-008 · observe:work
...
When the observation window closes, run `klc ack KLC-008 --pick 1` (clean), `--pick 2` (regression, auto-reopens build), or `--pick 3` (rollback).
...
`klc ack KLC-008 --pick <N>`, where N is:
