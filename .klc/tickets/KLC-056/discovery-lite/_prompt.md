# Agent prompt — KLC-056 · discovery-lite:work
...
work is done, the human runs `klc ack KLC-056` (with `--pick N` if
required) to confirm.
...
`klc ack KLC-056 --pick <N>`, where N is:
