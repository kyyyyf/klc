# Agent prompt — KLC-064 · observe:work
...
work is done, the human runs `klc ack KLC-064` (with `--pick N` if
required) to confirm.
...
When the observation window closes, run `klc ack KLC-064 --pick 1` (clean), `--pick 2` (regression, auto-reopens build), or `--pick 3` (rollback).
...
`klc ack KLC-064 --pick <N>`, where N is:
