# Agent prompt — KLC-059 · discovery-lite:work
...
work is done, the human runs `klc ack KLC-059` (with `--pick N` if
required) to confirm.
...
`klc ack KLC-059 --pick <N>`, where N is:
