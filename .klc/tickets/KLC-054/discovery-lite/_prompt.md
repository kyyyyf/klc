# Agent prompt — KLC-054 · discovery-lite:work
...
work is done, the human runs `klc ack KLC-054` (with `--pick N` if
required) to confirm.
...
`klc ack KLC-054 --pick <N>`, where N is:
