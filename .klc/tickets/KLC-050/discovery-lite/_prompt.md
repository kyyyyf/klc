# Agent prompt — KLC-050 · discovery-lite:work
...
work is done, the human runs `klc ack KLC-050` (with `--pick N` if
required) to confirm.
...
`klc ack KLC-050 --pick <N>`, where N is:
