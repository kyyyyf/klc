# Agent prompt — KLC-055 · discovery-lite:work
...
work is done, the human runs `klc ack KLC-055` (with `--pick N` if
required) to confirm.
...
`klc ack KLC-055 --pick <N>`, where N is:
